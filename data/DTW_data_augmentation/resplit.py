...
def resplitdatasets(datasetname, suffix="_ALT", ratio=.70):
    if os.path.exists('SmartForceps_Archive/{0}/{0}{1}_TRAIN'.format(datasetname, suffix)):
        print('WARNING TRAIN FILE EXISTS WITH SUFFIX:', suffix)
    if os.path.exists('SmartForceps_Archive/{0}/{0}{1}_TEST'.format(datasetname, suffix)):
        print('WARNING TEST FILE EXISTS WITH SUFFIX:', suffix)
    train_data = np.loadtxt('SmartForceps_Archive/{0}/{0}_TRAIN'.format(datasetname), delimiter=',')
    test_data = np.loadtxt('SmartForceps_Archive/{0}/{0}_TEST'.format(datasetname), delimiter=',')
