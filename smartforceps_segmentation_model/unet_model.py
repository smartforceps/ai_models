...
# (INPUT_CHANNELS, 1, 224）（INPUT_CHANNELS, 224, 224）
# pool_size=(1, 2), pool_size=(2, 2)
...
def FCN(inputsize=512, deconv_output_size=512, INPUT_CHANNELS=3, num_classes=6, filters=32):
    if K.image_data_format() == 'th':
        ...
        axis = 3
    conv1 = Conv2D(filters, 1, 3, border_mode='same', activation='relu')(inputs)
    pool1 = MaxPooling2D((1, 4), strides=(1, 2))(conv1)
    conv2 = Conv2D(filters, 1, 3, border_mode='same', activation='relu')(pool1)
    pool2 = MaxPooling2D((1, 4), strides=(1, 2))(conv2)
    conv3 = Conv2D(filters, 1, 3, border_mode='same', activation='relu')(pool2)
    pool3 = MaxPooling2D((1, 4), strides=(1, 2))(conv3)
    conv4 = Conv2D(filters, 1, 3, border_mode='same', activation='relu')(pool3)
    pool4 = MaxPooling2D((1, 4), strides=(1, 2))(conv4)
    fc = Conv2D(num_classes, 1, 1, border_mode='same', activation='relu')(pool4)
    dconv = Conv2DTranspose(num_classes, 1, 48, output_shape=(None, 1, deconv_output_size, num_classes),
                            subsample=(1, 16), border_mode='valid')(fc)
...
#     dconv = Deconvolution2D(num_classes, 1, 48, output_shape=(None, 1, deconv_output_size, num_classes),
#                             subsample=(1, 16), border_mode='valid')(fc)
#     conv_final = Activation('softmax')(dconv)
#     model = Model(inputs, conv_final, name="FCN")
#     return model
