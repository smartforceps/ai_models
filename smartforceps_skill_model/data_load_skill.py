...
def py_tsfeatures(df):
    ...
    # if len(set(peaks_left.tolist() + peaks_right.tolist())) == 0:
...
# def load_data(data_name='Smartforceps'):
#     data_output = []
#     try:
#         if data_name == 'Smartforceps':
#             data_output = load_Smartforceps_skill()
#
#     except:
...
def load_data(data_name='Smartforceps', subseq=224):
    ...
    try:
        if data_name == 'Smartforceps':
            data_output = load_Smartforceps_skill_3d(subseq)
    except:
        print('data not available')
...
def load_Smartforceps_skill():
    df = pd.read_csv('/home/amir/Desktop/smartforceps_ai_models/data/smartforceps_data/df_force_data_with_label.csv',
                     low_memory=False).iloc[:, [1, 2, 3, 7, 11]]

    df = df.dropna()

    label = df["SkillClass"].dropna().unique()

    # show how many training examples exist for each of the two states
    fig = plt.figure()
    df['SkillClass'].value_counts().plot(kind='bar',
                                         title='Data Stats by Skill Class')
    plt.ioff()
    plt.savefig(
        './results/graphs/class_distribution.png')
    # # plt.show()

    df.replace({'SkillClass': {'Expert': 1,
                               'Novice': 0}}, inplace=True)

    X = df[['Case', 'DataSection', 'LeftCalibratedForceValue', 'RightCalibratedForceValue']]
    y = np.array(df["SkillClass"])
    ...
    # one-hot encoding and calculating the class weights
...
def load_Smartforceps_skill_3d(window_size):
    df = pd.read_csv('/home/amir/Desktop/smartforceps_ai_models/data/smartforceps_data/df_force_data_with_label.csv',
                     index_col=0, low_memory=False).iloc[:, [0, 1, 2, 6, 10]]
    ...
    for seg in range(int(df.shape[0] / window_size)):
        ...
        # r = robjects.r
        # r['source']('/home/amir/Desktop/smartforceps_ai_models/data/smartforceps_data/r-feature-extraction.R')
        # # loading the function we have defined in R.
        # timeseries_feature_function_r = robjects.globalenv['timeseries_feature']
        # # converting it into r object for passing into r function
        # r_seg_forcedata = ro.conversion.py2rpy(df_seg)
        # # invoking the R function and getting the result
        # df_result_r = timeseries_feature_function_r(r_seg_forcedata)
        ...
        df_labels.append(int(df.iloc[(window_size * seg): (window_size * (seg + 1))][["SkillClass"]].median()[0]))
