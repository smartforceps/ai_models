...
import models_lstm
import models_incept
import data_load_person
import model_info
import common
# from smartforceps_dl_prediction_models_tf2.smartforceps_person_model import models_task
# from smartforceps_dl_prediction_models_tf2.smartforceps_person_model import data_load_person
# from smartforceps_dl_prediction_models_tf2.smartforceps_person_model import model_info
# from smartforceps_dl_prediction_models_tf2.smartforceps_person_model import common
import pandas as pd
import logging
import argparse
import time
...
read_processed_data = data_load_person.load_data(args.dataset, subseq=subseq)
...
optim_type = 'adam' #  # 'SGD'
sum_time = 0
hyper_params_list_len = 2
learning_rate_list = [0.0001, 0.001] #0.001
...
for i in range(len(learning_rate_list)):
    ...
    for j in range(hyper_params_list_len):
        ...
        if args.net == 'lstm_model':
            ...
            sub_model = models_incept.build_inception_model(
        ...
        # callbacks_list = [
        #     ModelCheckpoint("./results/person_model.model",
        #                     monitor='val_loss',
        #                     mode='min',
        #                     save_best_only=True,
        ...
        # ]
        callbacks_list = [
            ModelCheckpoint("./results/person_model_" + str(i) + str(j) + ".model",
                            monitor='val_loss',
                            mode='min',
                            save_best_only=True,
                            save_weights_only=True,
...
model.load_weights('./results/person_model_' + str(min_val_loss_idx[0].tolist()[0]) +
                   str(min_val_loss_idx[1].tolist()[0]) + '.model')
# serialize model to JSON
model_json = model.to_json()
with open("./results/person_model.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("./results/person_model.h5")
